...
function iTri(n){
    ...
    const between = (min,x,max) => min < x && x <= max;

    const cheers = [{start:'Starting Line... Good Luck!' },
                    {Swim:`${(140.6 - n).toFixed(2)} to go!`},
                    {Bike:`${(140.6 - n).toFixed(2)} to go!`},
                    {Run:`${(140.6 - n).toFixed(2)} to go!`},
                    {Run:`Nearly there!`},
                    {end:'You\'re done! Stop running!'}]

    return n===0 ? cheers[0]['start'] : between(0,n,2.4) ? cheers[1] : between(2.4, n , 114.4) ? cheers[2] : between(114.4, n , 130.6) ? cheers[3] : between(130.6, n, 140.6) ? cheers[4] : cheers[5]["end"];
    
}


console.log(iTri(0));
console.log(iTri(2.4));