...
const ticker = (text, width, tick) => {
    ...
    const whiteSpaces = new Array(width).fill(" ").join("");
    const content = whiteSpaces + text + whiteSpaces;
    const len = text.length;

    return content.substr((tick%len+len)%len,width);
...
console.log(ticker('Beautiful is better than ugly.', 10, 4));



//cw's solutions:

const ticker2 = (text, width, tick) => {
  ...
  const ticker3 = (text,width,tick) => ( " ".repeat(width) + text + " ".repeat(width) ).substr( tick%(text.length+width), width ) ;